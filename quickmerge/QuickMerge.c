...
void distribuiElementosProcessosFromFile(Processo *processo)
{
    ...
    if(processo->my_rank == source)
    {
        ...
        if(numeros == NULL)
        {
            printf("\n%d: Nao foi possivel alocar memoria para ler o arquivo.\n", processo->my_rank);
            exit(0);
        }

        lerArquivo(processo, numeros);

        // UMA PARTE DA LISTA FICARÁ COM O PROCESSO RAIZ
        qtdadeNum = processo->my_rank * processo->elementosPorProcesso;
        ...
        // ORDENA A PRIMEIRA SUBLISTA RECEBIDA
        qsort(processo->list, processo->lengthList, sizeof(int), cmpfunc);

        // PROCESSO RAIZ GERA O VETOR DE PIVÔS (SPLITTER)
        int n = powCast(2,processo->dim);
        for(i = 1; i <= n - 1; i++)
            processo->splitter[i] = processo->list[(i*processo->elementosPorProcesso)/n]; 
        ...
        for(i = 0; i < processo->comm_sz; i++)
        {
            if(processo->flagProcessos[i] == ESTAVEL)
            {
                if(processo->my_rank != i)
                {
                    ...
                    MPI_Send(&processo->splitter[0], processo->comm_sz, MPI_INT, i, tag_splitter, processo->Comm_origem);
                }
    ...
    else
    {
        ...
        MPI_Recv(&processo->splitter[0], processo->comm_sz, MPI_INT, source, tag_splitter, processo->Comm_origem, MPI_STATUS_IGNORE);
...
void quickMerge(Processo *processo)
{
    ...
    */
    for(i = processo->dim-1, k = 0; i >= 0; i--, k++)
    {
        ...
        //VERIFICA QUAIS PROCESSOS DEVEM FALHAR NESSA RODADA DE ACORDO COM O CENARIO DE FALHAS
        detectaFalhos(processo);
        ...
        if(processo->flagProcessos[processo->my_rank] == ESTAVEL)
        {
            ...
            do
            {
                ...
                // DEFINE-SE O INDEX DO SPLITTER E PROCESSO RECEBE O PIVO
